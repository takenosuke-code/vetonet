"""
VetoNet OpenAI SDK Integration - Guard Orchestrator

Central class that wires all components for OpenAI's function calling pattern.
...
class VetoNetOpenAI:
    """Framework-level interception for OpenAI SDK function calls.

    Intercepts tool calls BEFORE execution, verifying the real
    parameters against the user's locked intent.

    Usage:
        veto = VetoNetOpenAI(api_key="veto_sk_live_xxx")
        veto.lock_intent("Buy a $50 Amazon gift card")

        response = client.chat.completions.create(model="...", tools=[...], messages=[...])
        results = veto.process_tool_calls(response, executors={"buy_item": buy_item})

        # Get tool messages for next turn
        tool_messages = veto.get_tool_messages(response, executors={"buy_item": buy_item})
    """
    ...
    def lock_intent_from_messages(self, messages: List[dict]) -> Optional[str]:
        """Auto-detect and lock intent from OpenAI messages list.
        ...
        for msg in messages:
            if msg.get("role") == "user":
                ...
                # OpenAI user content can be a string or list of content parts
                if isinstance(content, str) and content.strip():
                    self.lock_intent(content)
                    return content
                # Handle content parts (vision, multi-part)
    ...
    def register_tool(
        ...
        Args:
            name: Tool name (must match the name in OpenAI's function definition)
    ...
    def get_tool_messages(
        ...
        """Process tool calls and return OpenAI-formatted tool messages.
    ...
    async def __aenter__(self) -> "VetoNetOpenAI":
        return self

    async def __aexit__(self, exc_type, exc_val, exc_tb) -> None:
        await self.close()

    def __enter__(self) -> "VetoNetOpenAI":
        return self
